...
class Sampler:
    def __init__(self, model: nn.Module, data: torch.utils.data.Dataset, config: SamplerConfig):
        self.config = config
        self.model = model
        ...
        self.data = data
        self.loader = torch.utils.data.DataLoader(data, batch_size=config.batch_size, shuffle=True)
    ...
    def sample(self, metrics: Optional[Dict[str, Metric]]=None, summary_fn: Callable=lambda **kwargs: kwargs):
        self.ensemble.train()
        ...
        num_steps = self.config.num_draws_per_chain * self.config.num_steps_bw_draws + self.config.num_burnin_steps
        ...
        for i, (xs, ys) in zip(range(num_steps), itertools.cycle(self.loader)):
            for j, model in enumerate(self.ensemble):
                ...
                if i >= self.config.num_burnin_steps and i % self.config.num_steps_bw_draws == 0:
        ...
        return summary_fn(loss_init=loss_init, num_samples=self.num_samples, **metrics)
...
def estimate_rlct(loss_init, losses, num_samples: int, **_):
    ...
    """
    loss_avg = losses.mean()
    rlct_estimate = (loss_avg - loss_init) * num_samples / torch.log(num_samples)
