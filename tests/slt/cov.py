...
def test_full_model(dummy_model):
    ...
    def update_model(model):
        ...
        model.fc1.weight.data = torch.tensor(new_params[:10].reshape((10,)), dtype=torch.float32)
        model.fc2.weight.data = torch.tensor(new_params[10:].reshape((10,)), dtype=torch.float32)

    acc = CovarianceAccumulator(num_weights=20, accessors=[accessor_layer1, accessor_layer2])
...
def test_within_head_covariance(dummy_transformer):
    ...
    acc = WithinHeadCovarianceAccumulator(num_heads, num_weights_per_head, [accessor_transformer_layer1, accessor_transformer_layer2])
    ...
    for l, actual_cov_matrix in enumerate([cov_matrix_layer_1, cov_matrix_layer_2]):  # 2 layers
        for h in range(num_heads):  # 2 heads
            local_cov_matrix = cov_matrix[l, h]
            actual_local_cov_matrix = actual_cov_matrix[h*num_weights_per_head:(h+1)*num_weights_per_head, h*num_weights_per_head:(h+1)*num_weights_per_head]
            local_mse = np.sum((local_cov_matrix - actual_local_cov_matrix) / local_cov_matrix.shape[0]) ** 2
